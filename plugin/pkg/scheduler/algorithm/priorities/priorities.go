...
func ImageLocalityPriority(pod *api.Pod, podLister algorithm.PodLister, nodeLister algorithm.NodeLister) (algorithm.HostPriorityList, error) {
	...
	for nodeName, sumSize := range sumSizeMap {
		result = append(result, algorithm.HostPriority{Host: nodeName,
...
func checkContainerImageOnNode(node api.Node, container api.Container) int64 {
	for _, image := range node.Status.Images {
		for _, repoTag := range image.Names {
